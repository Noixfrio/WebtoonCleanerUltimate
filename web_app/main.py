...
async def ultra(request: Request):
    return templates.TemplateResponse(request, "ultra_index.html", {})
...
async def api_ultra_inpaint(req: UltraInpaintRequest):
    try:
        ...
        try:
            ...
            logger.info("Processamento Ultra Inpaint concluído com sucesso")
        except InpaintServiceError as ise:
            # Erro do serviço de inpainting (ex: falta de espaço, modelo não carregado)
            logger.error(f"Falha no serviço Ultra Inpaint: {str(ise)}")
        ...
        logger.error(f"Erro inesperado no Ultra Inpaint API: {str(e)}")
        import traceback
        traceback.print_exc()
